<h1 align="center">Automated OS Install Tool for Cisco Intersight</h1>
...
3. [_Optional_] If you already have the Cisco Intersight SDK for Python installed, you may need to upgrade. An upgrade can be performed by running the following command:
   ...
   ```
4. Clone or download the **Automated OS Install Tool for Cisco Intersight** repository by using the ![GitHub Code Button](./assets/GitHub_Code_Button.png "GitHub Code Button") link on the main repository web page or by running the following command from the target directory if Git is locally installed:
    ```
    git clone https://github.com/ugo-emekauwa/intersight-os-installer
    ```
...
## How to Use
1. Please ensure that the above [**Prerequisites**](https://github.com/ugo-emekauwa/intersight-os-installer#prerequisites) have been met.
2. Within the unzipped **Automated OS Install Tool for Cisco Intersight** repository, navigate to the intersight_os_installer.py file.
3. Edit the intersight_os_installer.py file to set the **`key_id`** variable using the following instructions:

    **(a).** Open the intersight_os_installer.py file in an IDLE or text editor of choice.
    ...
    **(d).** Fill in between the quotes of the **`key_id`** variable value with the ID of your API key. For example:
      ...
      ```
4. Edit the intersight_os_installer.py file to set the **`key`** variable using the following instructions:
    ...
    **(d).** Fill in between the quotes of the **`key`** variable value with your system's file path to the SecretKey.txt file for your API key. For example:
      ...
      ```
5. Edit the intersight_os_installer.py file to set all the configuration variable values using the following instructions:
